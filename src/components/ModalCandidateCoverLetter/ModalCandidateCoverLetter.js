...
const CandidateCoverLetter = (props) => {
  ...
  const [coverLetter, setCandidateCoverLetter] = useState('');
  const [cvLink, setCvLink] = useState('');

  const handleClose = () => {
    gContext.toggleCandidateCoverLetterModalClose();
  };

  useEffect(() => {
    if (props.jobPostId == -1) {

    } else {
      CandidateServiceIml.viewApplication(props.jobPostId).then((response) => {
        if (response.data.errCode) {
        } else {
          setFullname(response.data.data.fullName);
          setEmail(response.data.data.email);
          setPhone(response.data.data.phone)
          setCandidateCoverLetter(response.data.data.coverLetter);
  ...
  return (
    <ModalStyled
      size="lg"
      centered
      show={gContext.candidateCoverLetterModalVisible}
      onHide={gContext.toggleCandidateCoverLetterModal}
    >
